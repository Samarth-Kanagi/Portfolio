    ...
    <section id="projects" className="py-24 px-5 sm:px-8">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-16">
          ...
          <p className="section-subheading">Real-world applications I've built from the ground up.</p>
        </div>
          ...
          {projects.map((project, index) => (
            ...
            >
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-4">
                <div>
                  <h3 className={`text-xl sm:text-2xl font-bold ${dark ? 'text-white' : 'text-gray-900'}`}>{project.title}</h3>
                  <p className="text-brand-500 font-medium text-sm">{project.subtitle}</p>
                </div>
                <span className={`text-xs font-medium whitespace-nowrap mt-1 sm:mt-0 ${dark ? 'text-gray-500' : 'text-gray-400'}`}>{project.period}</span>
              </div>
              ...
              <ul className="space-y-2 mb-6">
                {project.points.map((point, i) => (
                  <li key={i} className={`flex gap-2 text-sm leading-relaxed ${dark ? 'text-gray-400' : 'text-gray-600'}`}>
              ...
              <div className="flex gap-3">
                <a href={project.demo} className="btn-primary text-sm px-5 py-2">
                  <ExternalLink size={14} />
                  <span className="hidden sm:inline">Live Demo</span>
                  <span className="sm:hidden">Demo</span>
                </a>
                <a href={project.github} target="_blank" rel="noreferrer" className="btn-outline text-sm px-5 py-2">
                  <Code size={14} />
